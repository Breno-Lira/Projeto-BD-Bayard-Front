...
export default function AddCalcado() {
    ...
    const onSubmit = async (e) => {
        ...
        // Validação simples
        const { genero, tamanho, faixaEtaria, produto } = calcado
        if (!genero || !tamanho || !faixaEtaria || !produto.nome || !produto.preco) {
            alert("Por favor, preencha todos os campos obrigatórios.")
            return
        }

        await axios.put(`http://localhost:8080/calcados/editar/${codigoParam}`, calcado)
        navigate("/calcado")
    }
    ...
    const loadCalcado = async () => {
        const result = await axios.get(`http://localhost:8080/calcados/${codigoParam}`)
        setCalcado(result.data)
    }
            ...
            <div className='row'>
                <div className='col-md-6 offset-md-3 border rounded p-4 mt-4 shadow'>
                    ...
                    <form onSubmit={onSubmit}>
                        <div className='mb-3'>
                            <label>Código</label>
                            <input type="text" className='form-control' name='codigo' onChange={onInputChange}
                                value={calcado.produto.codigo} required disabled/>
                            ...
                            <input type="number" step="0.01" className='form-control' name='preco' onChange={onInputChange}
                                value={calcado.produto.preco} required />
